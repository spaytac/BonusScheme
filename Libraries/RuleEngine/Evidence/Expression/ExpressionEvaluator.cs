...
namespace RuleEngine.Evidence
{
    ...
    public class ExpressionEvaluator
    {
                ...
                else if (s.type == Type.Operator)
                {
                    ...
                    switch (s.name)
                    {
                        ...
                        case "/":
                            ...
                            op3 = EvaluateArithmetic(op1, s.name, op2);
                            break;
                        //case "+":
                        //    op2 = (Symbol)operandStack.Pop(); //this operation requires two parameters
                        //    op1 = (Symbol)operandStack.Pop();
        ...
        // Changs by AK:
        // Made this changes for better compatibility
        private Symbol EvaluateArithmetic(Symbol op1, string operation, Symbol op2)
        {
            Symbol op3 = new Symbol();
            op3.type = Type.Value;
            object o1 = null;
            ...
            try
            {
                ...
                var o1Value = -1.0;
                var o2Value = -1.0;
                if (double.TryParse(o1.ToString(), out o1Value) && double.TryParse(o2.ToString(), out o2Value))
                {
                    switch (operation)
                    {
                        case "+":
                            {
                                replacement = o1Value + o2Value;
                                break;
                            }
                        case "-":
                            {
            ...
            {
                Debug.WriteLine(String.Format("ExpressionEvaluator {0} {1} {2} = {3}", o1, operation, o2, replacement));
            }
